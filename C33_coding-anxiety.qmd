...
- name: Tiago A. Marques 
  ...
  affiliation: 
    - name: Faculdade de Ciências da Universidade de Lisboa 
      department: Departamento de Biologia Animal city: Lisboa 
      country: Portugal 
    - name: Faculdade de Ciências da Universidade de Lisboa 
      department: Centro de Estatística e Aplicações 
      city: Lisboa 
      country: Portugal 
...
![Figure 1. An example of a humorous cartoon that normalises coding anxiety. (https://www.thesquarecomics.com/) TODO: attribute this properly and add alt-text](chapters/images/C33_coding-anxiety_humour.png)
...
Fun reduces anxiety in a wide variety of contexts, so why not for coding anxiety, too? Coding need not be boring! There are many resources that showcase how coding can be fun, reinforcing that, when coding is required, receptivity to it is higher than when it is framed as an anxiety inducing endeavour. **R** packages like [**memer**](https://github.com/sctyner/memer) and [**memery**](https://cran.r-project.org/web/packages/memery/index.html) allow meme creation from within **R**, while generative art packages can produce abstract paintings from code. The **R** package [**fun**](https://cran.r-project.org/web/packages/fun/index.html) makes games and activities accessible from within the R environment. As an example, the classic game *Minesweeper* can be played within **R** (Figure 4).


![Figure 4. Examples of how R coding can be fun. Top-left: A meme showcasing how memes can reduce code anxiety; this meme can be recreated with 2 lines of code using package "memer". Top-right: A painting created in "aRtsy", a generative art R package. Bottom-left: An instance of a minesweeper game created in package "fun", demonstrating the authors inability to play it. Bottom-centre: An example of an animal that can be drawn in R syntax using the "cowsay" package. Bottom-right: The "CatterPlots" package can be used to generate scatterplots that include cats.](chapters/images/C33_coding-anxiety_memes.png "Examples of how R coding can be fun"){fig-alt="TODO: Some alt text for accessibility." width=70%}

The examples above can be used to introduce, in an informal way, the concepts of functions and arguments. At the end of a long coding session, or even during one, package [**praise**](https://cran.r-project.org/web/packages/praise/index.html) can also come in handy. A call to its single function praise() returns positive feedback like "You are wonderful!" or "You are supreme!". These simple affirmations often put a smile on students' faces, and based on our experience, that is halfway towards reducing anxiety. Finally, if students just can’t code but want to “fake it till they make it”, the **R** function look_busy()in the [**player**](https://cran.r-project.org/web/packages/player/index.html) package will produce output on the console that will make anyone else believe students are working hard (showcase it, but discourage its use!). We have used several of these approaches and provide them as a starting point, but given **R** is a dynamic and ever-evolving environment, other fun and engaging options must live out there waiting to be used by teachers to help reduce code anxiety.
...
![Figure 5. Pair programming and the analogy of a driver and a co-pilot #TODO (to replace with final pic – TAM’s sorting it)](chapters/images/C33_coding-anxiety_pair.png "Pair-programming"){fig-alt="TODO: Some alt text for accessibility." width=70%} 
