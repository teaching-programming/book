...
Civil engineers have good logical ability, especially deduction, transforming and applying math equations, dealing with real dimensions, approximation and optimisation. They have a firm belief in the deterministic: an expected system for this input must yield that output.
...
$$ U_{cr} =\left\{  \begin{array}{ll} {0.19} \, (d_{50})^{0.1} \log_{10}\left(\frac{4h}{d_{90}}\right) & \text{for } \;  0.1 \le d_{50} \le 0.5 \text{  mm}; \\ {8.5} \, (d_{50})^{0.6} \log_{10} \left(\frac{4h}{d_{90}} \right) & \text{for } \; {0.5} \le d_{50} \le 2 \text{ mm}; \end{array} \right. $$
...
for i in range(1,nx):
  for j in range(1,ny):
    ...
    dzbdt[i][j] = -(1/(1-n)) * (dqxdx[i][j] + dqydy[i][j])
```
